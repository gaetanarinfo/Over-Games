const form2 = document.getElementById('formContact');
const input2 = form2.getElementsByTagName('input');
...
function checkFormContact(event) {
    ...
    const prenom = document.getElementById('prenomA');
    const bug = document.getElementById('bugA');
    const sujet = document.getElementById('sujetA');
    ...
    if (email.value.length == 0) {
        ...
        jQuery.ajax({
            ...
            data: { nom: $("#nomA").val(), prenom: $("#prenomA").val(), email: $("#emailA").val(), sujet: $("#sujetA").val(), bug: $("#bugA").val(), message: $("#messageA").val() },
            type: "POST",
