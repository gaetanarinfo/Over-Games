...
IconSelect.COMPONENT_ICON_FILE_PATH = "assets/images/bbcode/arrow.png";
...
function IconSelect($$elementID, $$parameters) {
  ...
  function _init() {
    ...
    if (!$$parameters) $$parameters = {}; //En üst elementi seç

    if (_View.setIconSelectElement($$elementID)) {
      //set parameters
      $$parameters = _Model.checkParameters($$parameters); //create UI

      var ui = _View.createUI($$parameters, $$elementID); //basıldığında göster/gizle
      ...
      _View.showBox(false); //Nesneye basıldığında gizlemeyi iptal et.
  ...
  this.setSelectedIndex = function ($index) {
    ...
    _View.iconSelectElement.dispatchEvent(new Event('changed')); //_View.showBox(false);
  ...
  _View.createUI = function ($parameters) {
    ...
    */
    ...
    _View.iconSelectElement.setAttribute('class', 'icon-select ml-1');
    ...
    _View.boxElement = document.createElement('div'); //_View.boxElement.setAttribute('class', 'box');
    ...
    return results; //trigger: created ( run setValues )
  };
  ...
  function _Model() {} //TODO: params değişkenini kaldır yeni oluştursun.
